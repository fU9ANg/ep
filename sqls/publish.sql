...
DROP PROCEDURE IF EXISTS publish;
DELIMITER //
CREATE PROCEDURE publish(IN _group_id int, IN _course_name varchar(200), IN _time varchar(200), IN _belongs int, IN _art int, IN _lang int, IN _comm int, IN _health int, IN _science int, IN _resPath varchar(200), IN _client_id int, IN _client_type int)
_start:
BEGIN
    ...
    if (_result > 0) then
        ...
        if (_client_type != 2 && _client_type != 4) then
        ...
        if (_client_type = 2) then
            ...
            if (_client_id > _max || _client_id < _min) then
                SELECT "[DEBUG]: the parameter client_id is error!" AS DEBUG_INFO;
                leave _start;
            end if;
        end if;
