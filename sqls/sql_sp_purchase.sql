...
DROP PROCEDURE IF EXISTS sql_sp_purchase;
DELIMITER //
CREATE PROCEDURE sql_sp_purchase(IN _book_id int, IN _author_id int, IN _client_type_name varchar(64), IN _purchase_id int, IN _purchase_type_name varchar(64))
_start:
BEGIN
    ...
    DECLARE _purchase_type_id INT DEFAULT 0;
    ...
    SELECT count(client_type_id) INTO rlt FROM client_type WHERE client_type_name=_purchase_type_name;
    if (rlt = 0) then
        SELECT "[DEBUG]: not found client_type_id in the client_type table." AS DEBUG_INFO;
        leave _start;
    end if;
    ...
    SELECT client_type_id INTO _purchase_type_id FROM client_type WHERE client_type_name=_purchase_type_name;
    ...
    VALUE (auid, ctid, coid, 0, _purchase_id, _purchase_type_id, 1);
END //
DELIMITER ;
