...
DROP PROCEDURE IF EXISTS sql_sp_largess;
DELIMITER //
CREATE PROCEDURE sql_sp_largess(IN _book_id int, IN _author1_id int, IN _client_type_name1 varchar(64), IN _author2_id int, IN _client_type_name2 varchar(64))
_start:
BEGIN
    ...
    WHERE author_id=_author1_id AND client_type_id=_client1_type_id AND course_id=_book_id AND issell=0;
    if (rlt != 0) then
        ...
        INSERT INTO author_course(author_id, client_type_id, course_id, issell, owner, owner_type_id, flag) VALUE (auid, ctid, coid, 0, _author2_id, _client2_type_id, 0);
        leave _start;
    end if; 

    /* 如果被赠送此书的拥有者是本人. */
    SELECT count(author_course_id) INTO rlt  FROM author_course
    WHERE owner=_author1_id AND owner_type_id=_client1_type_id AND course_id=_book_id AND issell=0;
    if (rlt != 0) then
        UPDATE author_course SET owner=_author2_id, owner_type_id=_client2_type_id WHERE owner_id=_author1_id AND owner_type_id=_client1_type_id AND issell=0;
        leave _start;
    end if;

    SELECT "[DEBUG]: insert or update statement error!" AS DEBUG_INFO;
    leave _start;
