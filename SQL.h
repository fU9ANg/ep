...
"SELECT s.number, s.student_id AS student_id, s.first_name AS first_name, s.last_name AS last_name, s.sex, s.age, s.birthday, s.account AS account, s.password AS password, p.last_name AS guard_last_name, p.first_name AS guard_first_name, p.telephone, s.address, n.province AS native_name, r.race_name AS race_name, cl.class_id, cl.class_name, gr.grade_id, gr.grade_name, sl.school_name FROM grade AS gr, parents AS p, student AS s, race AS r, native_place AS n, class AS cl, school as sl where sl.school_id=s.school_id AND gr.grade_id=cl.grade_id AND s.class_id=cl.class_id AND r.race_id=s.race_id AND s.native_place_id=n.native_id AND s.parents1_id=p.parents_id AND s.student_id=?"
...
"SELECT s.number, s.student_id AS student_id, s.first_name AS first_name, s.last_name AS last_name, s.sex, s.age, s.birthday, s.account AS account, s.password AS password, p.last_name AS guard_last_name, p.first_name AS guard_first_name, p.telephone, s.address, n.province AS native_name, r.race_name AS race_name, cl.class_id, cl.class_name, gr.grade_id, gr.grade_name, sl.school_name FROM grade AS gr, parents AS p, student AS s, race AS r, native_place AS n, class AS cl, school as sl where sl.school_id=s.school_id AND gr.grade_id=cl.grade_id AND s.class_id=cl.class_id AND r.race_id=s.race_id AND s.native_place_id=n.native_id AND s.parents1_id=p.parents_id AND s.account=?"
...
"SELECT s.student_id AS student_id, s.first_name AS first_name, s.last_name AS last_name, s.account AS account, s.password AS password, c.class_name AS class_name, r.img_path AS res_path FROM student AS s, class AS c, resource AS r WHERE s.class_id=c.class_id AND s.picture_id=r.resource_id AND c.class_id=?"
...
"SELECT t.teacher_id AS teacher_id, t.teacher_id AS number, t.last_name AS last_name, t.first_name AS first_name, t.sex AS sex, t.birthday AS birthday, t.age AS age, t.account AS account, t.password AS password, r.race_name AS race_name, re.img_path AS res_path, n.province AS native_name, s.school_id AS school_id, s.school_name AS school_name FROM teacher AS t, native_place AS n, race AS r, school AS s, resource AS re WHERE r.race_id=t.race_id AND t.native_place_id=n.native_id AND s.school_id=t.school_id AND t.picture_id=re.resource_id AND t.teacher_id=?"
...
"SELECT c.course_id, g.grade_name, c.course_name, c.course_type_id, ct.course_type_name, cg.group_name, c.language, c.art, c.community, c.health, c.science, c.resPath, c.iconPath, c.xmlPath FROM course_type AS ct, grade AS g, course AS c, course_group as cg, grade_course as gc WHERE g.grade_id=gc.grade_id AND gc.course_id=c.course_id AND c.course_type_id=ct.course_type_id AND c.group_id=cg.group_id AND g.grade_id=?"
...
#define SQL_GET_COURSE_LIST_IN_STORE                \
"SELECT ac.author_id AS auth_id, ct.client_type_id AS auth_type, ac.course_id AS book_id, g.group_id AS book_type, c.course_name AS book_name FROM course AS c, author_course AS ac, course_group AS g, client_type as ct WHERE c.course_id=ac.course_id AND c.group_id=g.group_id AND ac.client_type_id=ct.client_type_id AND ac.issell=1"

#define SQL_GET_COURSE_LIST_BY_OWNERID_AND_TYPE_PUBLIC   \
"SELECT ac.course_id AS book_id, g.group_id AS book_type, c.course_name AS book_name, ac.author_id AS auth_id, ct.client_type_id AS auth_type  FROM course AS c, author_course AS ac, course_group AS g, client_type as ct    WHERE ac.client_type_id=ct.client_type_id AND c.course_id=ac.course_id AND c.group_id=g.group_id AND ac.issell=0 AND ((ac.author_id=? && ac.owner=?) || ac.owner=?) AND ct.client_type_name=?"
...
#define SQL_GET_SCHOOL_ACCOUNT_LIST_BY_SCHOOL_ID    \
"select account,  client_type_id from teacher where school_id=? union select account,  client_type_id from student where school_id=? union select account,  client_type_id from headmaster where school_id=? union select account,  client_type_id from parents where school_id=?"
